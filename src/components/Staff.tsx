  ...
  const handleSaveStaff = (e: React.FormEvent) => {
    ...
    if (editingStaff) {
      setUsers(users.map(u => u.id === editingStaff.id ? {
        ...
        password: staffForm.password || u.password,
        role: staffForm.role,
            ...
            <div className="p-6 pb-4 bg-white border-b border-slate-100 flex justify-between items-center" dir="rtl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-400 mt-0.5">حسابات والبدلات الفردية لـ {selectedStaff.name}</p>
                </div>
              </div>
            ...
            <div className="p-6 overflow-y-auto max-h-[78vh] space-y-6">
              ...
              <div className="bg-[#f0f7ff]/40 p-5 rounded-[2rem] border border-blue-105/60 space-y-4">
                <div className="flex items-center gap-3 pb-3 border-b border-blue-100/50">
                  ...
                  <div>
                    <strong className="text-sm font-black text-slate-850 block">{selectedStaff.name}</strong>
                    <span className="text-[10.5px] font-medium text-slate-450 block text-slate-400">{selectedStaff.jobTitle || "موظف مبيعات وتغليف"}</span>
                  </div>
                </div>
              ...
              <div className="space-y-4 text-right">
                ...
                <div className="space-y-2">
                  {(!selectedStaff.variableTasks || selectedStaff.variableTasks.length === 0) ? (
                    ...
                    <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                      {selectedStaff.variableTasks.map(vt => {
