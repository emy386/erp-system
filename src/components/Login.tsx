  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    if (hasSupabase) {
      try {
        ...
        if (authError) {
          // If live auth fails, let's try local fallback in case of demo accounts like owner@kidzy.com
        ...
        if (data.user) {
          ...
          // Map profile to local User type
        ...
        setError(err.message || "عفواً، البريد الإلكتروني أو كلمة المرور غير صحيحة.");
        setLoading(false);
        return;
      }
    }
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (hasSupabase) {
      try {
        ...
        setError(err.message || "فشل تسجيل الحساب، يرجى المحاولة بقيم أخرى أو كلمة مرور أطول.");
        setLoading(false);
        return;
      }
    }
      ...
      <div className="max-w-md w-full space-y-8 bg-white/90 backdrop-blur-xl p-8 sm:p-10 rounded-[2.5rem] border border-slate-100 shadow-premium hover:shadow-premium-hover transition-all duration-300 flex flex-col items-center z-10">
        ...
        <div className="pt-2">
          <p className="text-center text-[10px] text-slate-400 font-bold leading-relaxed">
            جميع الحقوق محفوظة لبراند كيدزي © {new Date().getFullYear()}
          </p>
        </div>
      </div>
    </div>
