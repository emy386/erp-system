...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  const filteredItems = menuItems.filter(item => {
    ...
    // If permissions array is empty or undefined, show nothing (user has no access)
  ...
  return (
    <aside 
      className={`fixed inset-y-0 right-0 w-64 bg-white border-l border-slate-100 flex flex-col z-[50] transition-transform duration-300 lg:translate-x-0 outline-none text-right ${
        isOpen ? "translate-x-0 shadow-2xl shadow-slate-100" : "translate-x-full"
      }`}
      dir="rtl"
    >
      {/* Brand logo bar */}
      <div className="p-6 border-b border-slate-100 flex items-center justify-between col-span-1">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight leading-none">Kidzy</h1>
