...
import { 
  Plus, Search, Calendar, Filter, Sparkles, AlertCircle, FileText, 
  MapPin, Phone, User, Check, Eye, Trash2, Edit2, Upload, MessageSquare, 
  X, CheckCircle, Clock, Truck, ShieldAlert, Heart, RefreshCw, ShoppingCart, Folder
} from 'lucide-react';
...
const ORDER_SOURCES = ["فيسبوك", "إنستغرام", "تيك توك", "واتساب", "مكالمة هاتفية", "الموقع الإلكتروني", "أخرى"];

const STATUS_DETAILS: Record<OrderStatus, { label: string; color: string; icon: any }> = {
...
export const Orders: React.FC = () => {
  ...
  // AI Extraction logic (Server Proxied API)
  const handleExtractWithAI = async (sourceContent: string | { mimeType: string; data: string }) => {
    setIsAiLoading(true);
    try {
      const response = await fetch("/api/extract-order", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ source: sourceContent })
      });

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || "فشل النظام في استخراج التفاصيل باستخدام محرك Gemini.");
      }

      const extracted = await response.json();

      // Transform extracted items list into order format items indices
      const transformedItems = (extracted.items || []).map((extItem: any) => {
        // Find if there's an existing product with fuzzy search matching
      ...
      setFormState(prev => ({
        ...
        childName: extracted.childName || prev.childName,
        customerPhone: extracted.customerPhone || prev.customerPhone,
        customerPhone2: extracted.customerPhone2 || prev.customerPhone2,
        governorate: extracted.governorate || prev.governorate,
        address: extracted.address || prev.address,
        discount: extracted.discount || prev.discount || 0,
        deliveryDuration: extracted.deliveryDuration === "urgent" ? "urgent" : "normal",
        notes: extracted.notes || prev.notes,
        // Match items
        items: transformedItems.length > 0 ? transformedItems : prev.items
      }));

      alert("تم استخراج بيانات المحادثة وتعبئتها أوتوماتيكياً بنجاح! 🤖✨ الرجاء المراجعة للتثبيت.");
    } catch (err: any) {
      console.error(err);
      alert(err.message || "عذراً، فشل معالج الذكاء الاصطناعي في تحليل البيانات.");
    } finally {
  ...
  const handleDropScreenshot = async (e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setDragActive(false);
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      ...
      if (file.type.startsWith("image/")) {
        ...
        reader.onloadend = () => {
          ...
          // Split base64 segments
          const [header, data] = base64.split(";base64,");
          const mimeType = header.replace("data:", "");
          
          // Ask user to run extraction
          if (confirm("هل تريد معالجة اللقطة المرفقة أوتوماتيكياً واستخراج بيانات الأوردر بالذكاء الاصطناعي؟")) {
            handleExtractWithAI({ mimeType, data });
          }
        };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      reader.onloadend = () => {
        ...
        if (confirm("هل تريد معالجة الصورة المرفقة واستخلاص قيم الأوردر بالذكاء الاصطناعي؟")) {
          handleExtractWithAI({ mimeType, data });
        }
      };
      reader.readAsDataURL(file);
  ...
  const handleOpenEdit = (ord: Order) => {
    ...
    setFormState({
      ...
      address: ord.address,
      items: mappedItems,
      shippingAmount: ord.shippingAmount,
  ...
  const urgentOrders = useMemo(() => {
    return filteredOrders.filter(
      o => (o.isUrgent || o.deliveryDuration === "urgent" || o.deliveryDuration === "مستعجل") &&
    ...
    <div className="space-y-8 text-right pb-16 animate-in fade-in duration-300" dir="rtl">
                ...
                <tbody className="divide-y divide-slate-50 text-slate-700 font-bold font-sans">
                  {filteredOrders.length === 0 ? (
                    ...
                    filteredOrders.map(o => {
                      const isUrgent = o.isUrgent || o.deliveryDuration === "urgent" || o.deliveryDuration === "مستعجل";
      ...
      {viewingOrder && (() => {
        ...
        const updateCurrentOrder = (updater: (prev: Order) => Order) => {
          setOrders(prevOrders => prevOrders.map(o => o.id === currentRef.id ? updater(o) : o));
        ...
        const handleItemProductionStatusChange = (itemIdx: number, status: any) => {
                        ...
                        <div className="pt-3 border-t border-slate-100 grid grid-cols-2 gap-3.5">
                          ...
                          <div className="flex flex-col gap-1 text-right">
                            ...
                            <select
                              ...
                              onChange={e => handleItemProductionStatusChange(idx, e.target.value as any)}
          ...
          {modalType === "smart" ? (
            ...
            <div className="bg-white w-full max-w-2xl rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col max-h-[92vh] animate-in zoom-in-95 duration-200 text-right">
              ...
              <div className="p-8 pb-4 flex justify-between items-start gap-4">
                ...
                <div className="flex items-center gap-4">
                  <div>
                    ...
                    <p className="text-slate-400 font-bold text-xs mt-1 font-sans">الصق النص أو ارفع سكرين شوت لفهم البيانات تلقائياً</p>
                  </div>
              ...
              <div className="p-8 pt-2 space-y-6 overflow-y-auto max-h-[68vh]">
                ...
                {/* Separator - أو ارفع صورة */}
                <div className="relative py-2">
                  <div className="absolute inset-0 flex items-center" aria-hidden="true">
                    <div className="w-full border-t border-slate-100"></div>
                  </div>
                  <div className="relative flex justify-center text-xs">
                    <span className="bg-white px-4 text-[11px] font-black text-slate-400">أو ارفع صورة</span>
                  </div>
                </div>

                {/* 2. Drag & Drop photo area */}
                <div 
                  onDragEnter={handleContainerDrag}
                  onDragOver={handleContainerDrag}
                  onDragLeave={handleContainerDrag}
                  onDrop={handleDropScreenshot}
                  className="border-2 border-dashed border-slate-200 hover:border-emerald-500/30 rounded-[1.8rem] flex flex-col items-center justify-center p-8 text-center transition-all bg-slate-50/30 hover:bg-white relative cursor-pointer group min-h-[140px]"
                >
                  <input
                    type="file"
                    accept="image/*"
                  ...
                  {formState.screenshot ? (
                    <div className="space-y-3 w-full">
                      ...
                      <p className="text-[#10b981] font-extrabold text-xs">تم إرفاق صورة الأوردر بنجاح! 📸</p>
                      <button 
                        type="button" 
                    ...
                    <>
                      <div className="bg-slate-100/80 group-hover:bg-emerald-50 text-slate-450 group-hover:text-emerald-500 p-3.5 rounded-2xl transition-colors">
                      ...
                      <span className="text-slate-400 font-semibold text-[10px] block mt-1 select-none font-sans">صيغ الصور المدعومة: PNG, JPG, JPEG</span>
                    </>
                  )}
                </div>
              </div>
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-100 flex gap-3">
                ...
                <button
                  type="button"
                  disabled={isAiLoading || (!aiInputText.trim() && !formState.screenshot)}
                  onClick={async () => {
                    if (formState.screenshot) {
                      const base64Str = formState.screenshot;
                      const [header, data] = base64Str.split(";base64,");
                      const mimeType = header.replace("data:", "");
                      await handleExtractWithAI({ mimeType, data });
                    } else {
                      await handleExtractWithAI(aiInputText);
                    }
                    setModalType("manual");
                  }}
                  className={`w-2/3 py-3.5 rounded-2xl text-xs font-black transition-all flex items-center justify-center gap-2 ${
                    isAiLoading 
                      ...
                      : !aiInputText.trim() && !formState.screenshot
                        ? "bg-[#cbd5e1] text-white cursor-not-allowed"
                        : "bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-100 cursor-pointer"
                  }`}
                >
                  <Sparkles size={14} className={isAiLoading ? "animate-spin animate-pulse" : "animate-pulse"} />
                  <span>{isAiLoading ? "جاري التحليل واستخراج البيانات..." : "استخراج وحفظ الأوردر 🪄"}</span>
                </button>
              </div>
            </div>
          ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                  ...
                  <div className="space-y-1.5 text-right">
                    ...
                    <input
                      ...
                      onChange={e => setFormState({ ...formState, shippingAmount: parseInt(e.target.value) || 0 })}
                    />
                  </div>
              ...
              <div className="bg-[#ecfdf5] border border-[#a7f3d0] rounded-3xl p-5 flex justify-between items-center text-right font-sans">
                <div className="space-y-1">
                  ...
                  <span className="text-[10px] text-emerald-600 block font-semibold">
                    سعر قطع الأوردر ({formState.items.reduce((sum, item) => sum + (item.price * item.quantity), 0)} ج.م) + الشحن ({formState.shippingPaid ? "0 ج.م" : `${formState.shippingAmount} ج.م`}){formState.discount > 0 ? ` - الخصم (${formState.discount} ج.م)` : ""}
                  </span>
                </div>
