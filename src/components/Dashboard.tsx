...
export function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const filteredOrdersList = (orders || []).filter(o => o.creationDate && inRange(o.creationDate) && o.status !== "cancelled");
    ...
    const totalMfgCosts = filteredOrdersList.reduce((sum, o) => {
      const orderCost = (o.items || []).reduce((itemSum, item) => {
        const prod = products.find(p => p.id === item.productId);
        const basicCost = prod ? ((prod.materialsCost || 0) + (prod.workshopFee || 0)) : 0;
        
        // Item custom markup adjustment if price is higher than standard selling price
        const markup = (prod && item.price > prod.sellingPrice) ? (item.price - prod.sellingPrice) : 0;
        const totalUnitCost = basicCost + markup;
              ...
              <div className="p-4 bg-slate-50/70 rounded-2xl border border-slate-100 space-y-2 text-right">
                ...
                <div className="space-y-1.5 pt-1 text-slate-600 font-sans">
                  ...
                  <p className="flex justify-between pt-1 border-t border-slate-200/50 text-slate-800 font-black">
                    ...
                    <span className="text-blue-600">{selectedOrder.total.toLocaleString()} ج.م</span>
                  </p>
                </div>
              </div>
            </div>
