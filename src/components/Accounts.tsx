...
import { GeneralExpense, OrderStatus } from '../types';
  ...
  const stats = useMemo(() => {
    ...
    const monthMfgCost = monthOrders.reduce((sum, order) => {
      const orderItemsMfgCost = (order.items || []).reduce((itemAcc, item) => {
        ...
        const unitRealMfgCost = unitMfgCost + priceDiff;
        return itemAcc + (unitRealMfgCost * item.quantity);
      }, 0);
    ...
    // Helper to calculate actual order revenue (excluding returns)
    const getOrderRevenue = (o: any) => {
      if (o.status === 'cancelled' || o.status === 'returned') return 0;
      const activeItemsTotal = (o.items || [])
        .filter((i: any) => !i.isReturned && i.productionStatus !== 'cancelled')
        .reduce((sum: number, i: any) => sum + (i.price * i.quantity), 0);
      const shippingPaidVal = o.shippingPaid;
      const shippingAmtVal = o.shippingAmount || 0;
      const discountVal = Number(o.discount) || 0;
      return activeItemsTotal + (shippingPaidVal ? 0 : shippingAmtVal) - discountVal;
    };
